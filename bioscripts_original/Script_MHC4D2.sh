...
if test -z "$2"
	then
		...
		echo -e "\n  Nota2: Renomeando pasta para $name-D2."		
		echo -e "\n<<<<<<<<<<<<<<<<<<< NBLI >>>>>>>>>>>>>>>>>>>>>>\n"
	else
    #Prosseguir com o script#
		name=$2
...
rm \#*
cp /usr/local/bioscripts/biofiles/MVina_CESUP.sh $name-D2/
cd $name-D2/ 
			...
			mv donor_HLA-B2705.pdb donor_MHC.pdb
			    ...
			    mv donor_H2Kb.pdb donor_MHC.pdb
			            ...
			            mv donor_H2Db.pdb donor_MHC.pdb
                        		...
                        		echo -e "\n   Nota1: Vina_config não foi copiado para a pasta $name-D2."
...
rm donor_MHC.pdb
/usr/local/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py -r MHC_4D2.pdb -A -o
#Prosseguir para geração do ep_4D2.pdbqt;
/usr/local/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py -l ep_4D2.pdb -B "None"   
#
echo -e "\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NBLI >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
echo -e "\n     Minimização do complexo \"$allele\" realizada com sucesso."
