...
gmx editconf -f target -c -d 1.5 -bt cubic -o target_box
gmx solvate -cp target_box -cs -p target_TOPOLOGY -o target_box_sol
gmx grompp  -f $DOCKTOPE_PATH/em10000ps -c target_box_sol -p target_TOPOLOGY -o targetmdrun_EM1 -r target_box_sol
gmx mdrun -nt $2 -v -s targetmdrun_EM1 -o target_traj_EM1 -c Final_target_BOX-EM -g target_log_EM1 -e energia_EM1.edr
echo -e "1\r" | gmx trjconv -s targetmdrun_EM1.tpr -f Final_target_BOX-EM.gro -o EM-OUT_NW.pdb
if test -z "$3"
    then
        echo -e "Completed Energy Minimization of $1 (output without waters: EM-OUT_NW.pdb)"
    else
        gmx editconf -f EM-OUT_NW.pdb -o peptide.pdb -label $3
